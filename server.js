...
models.questionnaire.findAll().then((qn) => {
  if (qn != null) QusetionnaireID_def = qn[0].id.toString();
});
...
let getR = function (req, res) {
    ...
    models.questionnaire
      ...
      .then((qn) => {
        if (qn != null) {
          if (!qn.anonymous && !login) return Promise.reject(403);
        ...
        if (a != null) {
          ...
          if (total != 0) {
            for (i = 0; i < IoT_json.percentage.length; i++)
              IoT_json.percentage[i] = Math.round(
                (IoT_json.percentage[i] * 100) / total
              );
          }
  ...
  getNxtQ = function (req, res) {
    ...
    models.questionnaire
      ...
      .catch((code) => {
        ...
        else if (code == 200) response.getNextQuestion(res, nextQ);
      });
  },
  ...
  postQ = function (req, res) {
    if (req.body.answers.length !== 0) {
      ...
      req.body.uuid = genUUID.default();
      models.question
        .create(req.body, { include: [models.answer] })
        .then((q) => {
          // let d = dai(q.dataValues.id, q.dataValues.uuid, q.dataValues.answers);
          //daList.push(d);
          //d.register();
    ...
    } else response.getBadRequest(res);
  },
...
let index = function (req, res) {
    if (QusetionnaireID_def != undefined) {
      ...
      // list all question
      models.question.findAll().then((qList) => {
  ...
  vote = function (req, res) {
    ...
    models.questionnaire
      ...
      .then((qn) => {
        questionnaire = qn;
        queryObj = { questionnaireId: id };
        if (qn == null) return Promise.reject(404);
        else if (!qn.anonymous && !login) return Promise.reject(200);
        else {
          return models.question.findAll({
            where: queryObj,
            order: [["id", "ASC"]],
      ...
      .then((q) => {
        if (q == null) return Promise.reject(404);
        else {
          question = q[0];
      ...
      .then((v) => {
        vote = v;
        if (v != null) return models.answer.findByPk(v.answerId);
        else return Promise.reject(200);
      })
      .then((va) => {
        voteAnswer = va;
        return Promise.reject(200);
    ...
    models.question
      ...
      .then((a) => {
        ...
        a.forEach((answer) => {
          ...
          color.forEach((c, idx) => {
            if (c === answer.color) options[idx] = answer.description;
          });
        });
  ...
  adminquestion = function (req, res) {
    ...
    models.question.findAll({ where: queryObj }).then((qList) => {
      response.getAdminQuestionListPage(res, qList);
    });
  },
...
app.get(
  "^/admin/questionnaire/:qnid([0-9]+)/edit/:id([0-9]+)(/){0,1}$",
  bAuth,
  adminEdit
);
...
} else var socketclient = require("socket.io-client")("http://localhost");
...
let pollNext = function (req, res) {
  nextQId =
    req.params.id == undefined ? parseInt(curQuestionIdx) + 1 : req.params.id;
