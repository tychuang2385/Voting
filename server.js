...
models.question.findAll({ include: [models.answer] }).then((question) => {
...
let getR = function (req, res) {
    ...
    models.answer.findAll({ where: queryObj }).then((a) => {
      let ratio = [],
        total = 0,
        qRatio;
      if (a != null) {
  ...
  postQ = function (req, res) {
    if (req.body.options.length !== 0) {
      ...
      models.question.create(q, { include: [models.answer] }).then((q) => {
        let d = dai(q.dataValues.id, q.dataValues.uuid, q.dataValues.answers);
        daList.push(d);
        d.register();
        ...
        if (a) {
          ...
          if (!question.anonymous && login) {
            ...
            for (let i = 0; i < daList.length; i++)
              if (daList[i].mac === question.uuid) daList[i].push(a);
      ...
      .then((v) => {
        if (v == null) {
          ...
          for (var i = 0; i < daList.length; i++)
            if (daList[i].mac === question.uuid) daList[i].push(answer);
          models.vote.create({
            userId: req.user.id,
            questionId: id,
            answerId: answer.id,
        ...
        } else return Promise.reject(409);
      })
      .catch((code) => {
        if (code === 200) response.getSuccess(res);
        else if (code === 401) response.getPermissionDenied(res);
        else if (code === 404) response.getBadRequest(res);
  ...
  vote = function (req, res) {
    ...
    models.question
      ...
      .then((q) => {
        ...
        if (0) {
        } else if (q == null) return Promise.reject(404);
          ...
          return models.answer.findAll({ where: queryObj });
        else if (!q.anonymous && !login) return Promise.reject(200);
      })
      .then((a) => {
        options = [];
        answer = a;
    ...
    models.question
      ...
      .then((a) => {
        ...
        a.forEach((answer) => {
          ...
          color.forEach((c, idx) => {
            if (c === answer.color) options[idx] = answer.option;
          });
        });
