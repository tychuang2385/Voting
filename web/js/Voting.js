    ...
    success: function (nxtQ) {
      if (nxtQ.question.questionIdx === questionIdx - 1) {
        ...
        jQuery.fn.slideLeftHide = (function (speed, callback) {
          ...
          $("#question").html(
            ejs.render(question, { q: nxtQ.question.description })
          );
...
$(document).ready(function () {
  ...
  socketIo.on("START", (curQ) => {
    if (id !== curQ.questionnaireIdx) return;
    questionIdx = 0;
    startPoll();
  });
  socketIo.on("CUR_Q", (curQ) => {
    ...
    if (questionIdx === curQ.questionIdx) return;
  ...
  socketIo.on("NEXT", (curQ) => {
    ...
    pauseVoted = curQ.pauseVoted;
    questionIdx = curQ.questionIdx;
    if (isStart && !isEnd) getNextQuestion();
  });
