        ...
        (function (index) {
          $("button").each(function () {
            if ($(this).css("background-color") == data.ratio[index].color) {
              var r =
                Math.round((data.ratio[i].count / data.total) * 1000) / 10;
              $(this)
                .find("span")
                .html(r.toString() + "%");
            }
...
$(document).ready(function () {
  ...
  if (
    ...
    if (anonymous) $("#chooseAswer").text("你的答案: " + answer);
    else $("#chooseAswer").text("你的答案: " + va);
    setRatio(id);
    return;
  } else if ((answer == "" && anonymous == true) || (va == undefined && anonymous == false)) $("button").prop("disabled", false);

  $("button").click(function () {
    //store user answer
    var text = $(this).data("datac");
    $("#chooseAswer").text("你的答案: " + text);
    if (anonymous) setCookie(id, encodeURIComponent(text), 30);
